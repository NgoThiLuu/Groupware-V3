...
def IsDisplayedByIdLogin():  #id log in random khai bao
  for i in range(2,7) :
    try :
      time.sleep(1)
      driver.find_element_by_id(":r%s:" % i)
      return i
    except NoSuchElementException:
      pass
  return False      #ket thuc id log in random khai bao
...
def log_in_domain(domain_name):
    ...
    # username = driver.find_element_by_id(":r5:")
    # username.send_keys(data["hanbiro_user"])
    # Logging("2. Input ID ")    
    # # frame_element = driver.find_element_by_id("iframeLoginPassword")
    # # driver.switch_to.frame(frame_element)
    # password = driver.find_element_by_id("gw_pass")
    # password.send_keys(data["hanbiro_password"])
    # driver.switch_to.default_content()
    # Logging("3. Input Password ")  
    # submit = driver.find_element_by_id(":r6:")
    # submit.send_keys(Keys.RETURN)
    # Logging("4. Enter Login  ")
    # driver.implicitly_wait(1)


    Id = IsDisplayedByIdLogin()  #id log in random bat dau
    if  Id != False :
        driver.find_element_by_id(":r%s:" % Id).send_keys(data["hanbiro_user"])
    password = driver.find_element_by_id("gw_pass")
    password.send_keys(data["hanbiro_password"])
    driver.switch_to.default_content()
    Logging("3. Input Password ")  
    Id = IsDisplayedByIdLogin()
    if  Id != False :
        driver.find_element_by_id(":r%s:" % Id).send_keys(Keys.RETURN)  #id log in random ket thuc



    try:
        click_modal_home = driver.find_element_by_xpath(data["modal_home"])
        click_modal_home.click()
